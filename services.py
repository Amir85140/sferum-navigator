...
# Ключевые слова для быстрого определения режима (без запроса к ИИ)
MODE_KEYWORDS = {
    ...
    "tests": ["тест", "проверь", "экзамен", "викторин", "квиз", "вопрос"],
...
def _get_token() -> str:
    ...
    _token_cache["expires_at"] = now + 1700  # 28 минут (с запасом)
    print("✅ Токен получен и закэширован")
    return _token_cache["token"]

def detect_mode(text: str) -> str:
    """Определяет режим по тексту. Сначала ключевые слова, потом ИИ."""
    ...
    # Если не определилось — спрашиваем ИИ (быстрый запрос)
    ...
    try:
        ...
        response = requests.post(
            ...
            json={
                ...
                "max_tokens": 300,  # Уменьшено для скорости
                "temperature": 0.7
            },
            verify=False,
            timeout=20
